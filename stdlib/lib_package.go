...
const LUA_LOADED_TABLE = "_LOADED"   // key, in the registry, for table of loaded modules
const LUA_PRELOAD_TABLE = "_PRELOAD" // key, in the registry, for table of preloaded loaders
...
func OpenPackageLib(ls LuaState) int {
	ls.NewLib(pkgFuncs) // 1. create package table
	createSearchersTable(ls)

	ls.PushString("./?.lua;./?/init.lua")
	ls.SetField(-2, "path") // 2. set path field

	ls.PushString(LUA_DIRSEP + "\n" + LUA_PATH_SEP + "\n" + LUA_PATH_MARK + "\n" + LUA_EXEC_DIR + "\n" + LUA_IGMARK + "\n")
	ls.SetField(-2, "config") // 3. set config field

	ls.GetSubTable(LUA_REGISTRYINDEX, LUA_LOADED_TABLE)
	ls.SetField(-2, "loaded") // 4. set loaded field

	ls.GetSubTable(LUA_REGISTRYINDEX, LUA_PRELOAD_TABLE)
	ls.SetField(-2, "preload") // 5. set preload field

	ls.PushGlobalTable()
	ls.PushValue(-2) // 6. set 'package' as upvalue for next lib

	ls.SetFuncs(llFuncs, 1) // 7. open lib into global table
	ls.Pop(1)               // 8. pop global table
